...
public class RoleProcessing 
{
	...
	static String className = RoleProcessing.class.getName();
	Logger logFile;
	
	public RoleProcessing(Logger logFile) {
	...
	public void queryPropertyIndividuals(String propertyIndividuals, String sparqlEndpoint) throws Exception {
		...
		logFile.log("INFO", className, "IRI Population found :");
		...
		while (rs.hasNext()) {
			...
			logFile.log("INFO", className, iriProp.toString());
		...
		logFile.log("INFO", className, "END QUERY PROPERTIES ...");
		qexec.close();
