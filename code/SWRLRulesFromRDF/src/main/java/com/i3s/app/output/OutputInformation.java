...
public class OutputInformation 
{
	...
	static String className = OutputInformation.class.getName();
     ...
     */
    public OutputInformation(String strOutputFileName, Logger logFile)
    {
    	OutputInformation.logFile = logFile;
    ...
    */
     ...
     * @param numRule id of rule
     * @param type could be true (if the text must be printed also in the Eclipse console), false otherwise.
     */
    public static void showPattern(Pattern pattern, int numRule, boolean type)
    {
        PrintStream output = OutputInformation.outputType(type);
        
        ArrayList<Atom> listAtoms = pattern.getListAtoms();
        logFile.log("INFO", className, "Rule n°" + (numRule + 1) + " : number of atoms in pattern: " + listAtoms.size());
